...
- **Jon itinerary six-point update (2026-09-20).** Chip via CoS, authorized Jon Ripchick facts — not a trip-optimizer export. **1.** Objective Normandy is Oct 21, not Oct 19: removed from Day 8 (transfer/check-in only); Day 10 is now meet **9:00 AM at the Bayeux apartment** (4 Rue Franche / Home in Bayeux), guide Elisha–Elisa / objectivenormandy.com kept. Mont-Saint-Michel filler + Pizza Sam stripped from Oct 21. **2.** Oct 22 depart Bayeux **11:00 AM** to drive to ORY for TP455 to OPO (TP455 16:50 kept). Official Airbnb check-out 12:00 PM still noted as the booking bound, not the departure clock. **3.** London Oct 12–18: checkout 10:00 AM Oct 18; explicit bags-may-stay-at-53-Greek-St note; Greek St→LHR transport moved **11:00 → 12:00**. **4.** TP211 date **confirmed Oct 26** — dropped `_dateUnconfirmed` / “assumed” / “confirm with Jon” hedging on Day 15, meta, ACTIONS, flight text. **5.** ORY hotel night of Oct 18 stays TBD — no property invented. **6.** Normandy restaurants/filler TBD except Objective Normandy on the 21st; Porto days’ booked structure kept (Porto exception). Day 9 is a single TBD note (no cathedral/museum/Le Volet itinerary). **Conflicts:** retired Day 8 American-Sector-vs-arrival banner and Day 11 noon-checkout squeeze — both claims are now false; `DAY_CONFLICTS` is empty (no invented replacements). BOOKING_ROW_RULES / ACTIONS / History / Transit / packing / SW `trip-cache-v7` updated in the same pass. `scripts/update_sep14_itinerary.py` would undo this — marked superseded. Local prove only — no live-site claim.

- **Day-strip concern highlight wired to `DAY_CONFLICTS` (2026-09-20).** Chip: “In ripshit highlight the day pill for any days there are concerns about the itinerary.” Conflict banners already existed for Day 8 (Mon Oct 19, American Sector vs Orly→Bayeux) and Day 11 (Thu Oct 22, noon Bayeux checkout → TP455 16:50 ORY); the sticky strip pills for those days were visually identical to every other day. `renderDayTabsBar` now adds `day-tab--concern` (plus a ⚠ `.day-tab-concern-mark` and an aria/title suffix from the conflict title) when `DAY_CONFLICTS[n]` is set — same source as `conflictBannerHTML`, no new trip facts. CSS: amber fill/border in the `.conflict-banner` family (`#ffecc0` / `#e6c36a` — a step stronger than the banner’s `#fff6e5` so a 44px phone pill still reads as flagged), city top-stripe kept, selected state keeps navy + amber ring so the flag stays visible. Non-concern pills, Jump/Today, calendar-date labels, and scroll-spy untouched. Local prove only — no live-site claim.
...
- **Jon Sep 14 2026 itinerary refresh — flights, stays, Wed/Thu London detail (2026-09-14).** Source is Jonathan Ripchick’s two emails that day on Chip’s “Please do not make a mockery” thread (plus the Aug city-level list), not a trip-optimizer export. **Flights:** UA940/UA145/CDG Air France are gone. In are BA184 EWR→LHR Oct 12 17:50/05:50 (Jon typed LHW; UI uses LHR and says so), BA8137 LHR→ORY Oct 18 15:00/17:35 (real BA codeshare, typically Vueling VY8961), TP455 ORY→OPO Oct 22 16:50/18:05, and TP211 OPO→EWR 19:30/23:55 with **no date in the email** — Day 15 uses Oct 26 from Porto checkout and labels it `Oct 26 assumed — confirm with Jon` (`_dateUnconfirmed`). Durations recomputed from those clock times + real Oct 2026 offsets (BA184 7h; BA8137 1h35m after UK/France offset; TP455 2h15m after France/Portugal offset; TP211 8h25m on an assumed Oct 26). **Stays:** 53 Greek St Airbnb HMFRRRZRTN (check-in 3pm / out 10am, Oct 12–18); ORY hotel Oct 18 TBD (unattributed night, same pattern as the old ferry night); Bayeux 4 Rue Franche HMKWYPDKBE (5pm/12pm, Oct 19–22); Porto Rua dos Mercadores 77, Ribeira São João 3rd floor HM549AK8C2 (4pm/11am, Oct 22–26; Jon wrote “Ribera San Joao”). **London days:** only Wed Oct 14 (IWM + Borough Market + Funky Noodle + Operation Mincemeat at Fortune Theatre 7:30pm) and Thu Oct 15 (Churchill War Rooms + Kensington walk; dinner TBD) are hour-by-hour from Jon. Oct 12/13/16/17 stay high-level. Funky Noodle is a real walk-in at 103 Charing Cross Road (not on Greek St; ~3 min as Jon said); their site says no reservations. **Logistics that were invented if kept:** ferry + UK rental + Tank Museum-on-Oct-18 are off the traveler-facing plan (Tank Museum remains an unscheduled London idea). Day 8 no longer assumes a dawn Ouistreham arrival or a same-day American Sector tour — Objective Normandy / Elisha (site: Elisa Denis, objectivenormandy.com) is on the Bayeux stay with the day unconfirmed. Bayeux↔ORY transfers both unspecified. Phantom/Hamilton/ACTIONS ferry+rental rows removed. Night math still 14 = 5 London + 1 Orly TBD + 3 Bayeux + 4 Porto + 1 overnight-flight. New pins for Greek St / Fortune / Funky Noodle / Borough Market / Mercadores 77 are address-level `approx` (no TomTom this pass). Hardcoded ACTIONS / History / packing / Normandy transit tip updated in the same pass — a data-only edit would have left UA940 and the ferry live.
